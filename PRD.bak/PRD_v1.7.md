...
| customer_memberships | `subscription_status`, `membership_tier`, 구독 시각, `created_at`, `updated_at` | 사람 단위 구독 상태·거래 실적 등급과 증분 수집 |
...
| customer_memberships | 사람별 최소 계정 `created_at` → `created_at`, `NON_MEMBER`·구독 시각 `NULL`, `--seeded-at` → `updated_at` |
...
Seed는 Olist 원본에 구독 이력이 없으므로 모든 사람을 `subscription_status='NON_MEMBER'`와
구독 관련 시각 `NULL`로 기록한다. `membership_tier`는 `customer_unique_id`별
`order_status='delivered'` 주문 수로 계산해 사람 단위 `customer_memberships`에 정확히 한 행으로 기록한다.
...
| `subscription_status` | VARCHAR(32) | NOT NULL, `NON_MEMBER/TRIAL/ACTIVE/PAYMENT_FAILED/CANCEL_REQUESTED/CHURNED` |
| `membership_tier`    | VARCHAR(16)  | NOT NULL, `BRONZE/SILVER/GOLD` |
...
| `customer_memberships.subscription_status` | `stg_customer_observations.subscription_status` |
| `customer_memberships.membership_tier` | `stg_customer_observations.membership_tier` |
| `customer_memberships` 구독 시각 | `stg_customer_observations`의 같은 이름 시각 |
...
Grain은 "한 행이 무엇 하나를 나타내는가"의 선언이다. Model마다 아래 문장을 Grain 정의로 삼고,
Unique Key는 그 문장을 데이터로 강제하는 수단이다. 새 컬럼이나 Measure를 추가할 때는 해당
문장이 여전히 참인지 먼저 확인한다.
...
- `fact_orders`: 한 행은 하나의 주문을 나타낸다. 주문에 속한 주문 상품과 결제는 이미 주문 단위로
  집계되어 이 행의 Measure에 반영되어 있다.
- `fact_order_items`: 한 행은 하나의 주문에 포함된 하나의 주문 상품(order item)을 나타낸다.
...
Delivery Measure는 `fact_orders`에 함께 둔다. 배송 Timestamp는 주문 1건당 각 1개이므로
"한 행은 하나의 주문을 나타낸다"는 `fact_orders`의 Grain 문장이 그대로 유지된다. 별도
`fact_delivery`를 만들면 `order_id`가 같은 1:1 Fact가 되어 같은 Grain을 두 Table로 쪼개므로
분리하지 않는다.
...
- 네 Measure는 Non-additive다. SUM 금지, 평균·분위수·비율로만 집계한다.
- 원천 Timestamp가 `NULL`인 주문은 해당 Measure도 `NULL`이다. 미배송 주문은 평균과 정시 배송률의 분모에서 제외한다.
- `delivery_delay_days`는 조기 배송에서 음수다. 지연 일수만 볼 때는 양수 구간으로 필터한다.
- `is_late`는 `delivered_at`이 `NULL`이면 `NULL`이다. 미배송을 지연으로 세지 않는다.
- 정시 배송률 분모는 `delivered_at IS NOT NULL`인 주문이다.
