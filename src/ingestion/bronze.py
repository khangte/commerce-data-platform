...
class BronzeWriteContext:
    """한 Table Batch의 Bronze 기술 컬럼에 반복 기록할 고정 값이다."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.source_table != "orders":
            raise ValueError("Orders Bronze Writer only supports the orders source_table")
...
def _orders_page_to_arrow_table(page: OrdersPage, context: BronzeWriteContext) -> pa.Table:
    """Raw-compatible Source Order Page와 기술 컬럼을 명시적 Arrow Schema로 변환한다."""
...
def _order_row(record: SourceOrderRecord, context: BronzeWriteContext) -> dict[str, object]:
    """Source Order와 고정 기술 컬럼을 Bronze Arrow Row 표현으로 변환한다."""
    return {
