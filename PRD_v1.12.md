...
v1.12는 구독 도메인 Source Schema를 재구조화한다. `customer_subscriptions`는
사람 Grain 단일 행 대신 구독 계약(`subscription_id`) Grain으로 바뀌고,
`subscription_payments`는 청구 회차·시도(`billing_cycle_sequence`,
`attempt_sequence`) Grain으로 바뀐다. Seed는 구독 이력이 없는 고객에 대해
`customer_subscriptions` 행을 만들지 않는다 (`NON_MEMBER` 상태 행 제거).

- `customer_subscriptions`: PK가 `customer_unique_id`에서 `subscription_id`로
  바뀐다. 자동갱신 여부(`auto_renew_enabled`), 청구 기한(`billing_due_at`)과
  실제 다음 결제 시도 예정 시각(`next_payment_attempt_at`)을 분리한다.
- `subscription_payments`: PK가 `(customer_unique_id, billing_sequence)`에서
  `payment_id`로 바뀐다. 결제수단·대행사·대행사 거래 ID, 실패 원인 코드,
  통화 코드를 추가한다.
- 증분 Cursor 컬럼이 `customer_unique_id` 기반에서 각 테이블의 신규 PK
  기반으로 바뀐다: `customer_subscriptions (updated_at, subscription_id)`,
  `subscription_payments (updated_at, payment_id)`.
- `src/ingestion/orphan.py`: 미정의 `_utc_datetime` 호출을
  `manifest._utc_datetime_from_manifest`로 수정. `logical_date` 문자열 파싱
  실패로 Orphan 재조정이 항상 실패하던 버그 해결.
...
- 상시 참조 성격의 문서를 `docs/reference/`로 모은다. 데이터 변환 흐름 문서가
...
- 자동갱신은 청구 기한(`billing_due_at`)과 실제 다음 결제 시도 예정 시각(`next_payment_attempt_at`)을 분리한다.
- Source Table은 7개에서 9개다. 구독 계약이 없는 고객은 `customer_subscriptions` 행을 만들지 않는다.
...
다음 세 CSV는 `scripts/download_dataset.py`가 Dataset 전체를 내려받으므로 `data/raw/olist/`에 존재하지만, Seed·Bronze 수집·Warehouse 대상이 아니다. V1 재논의 대상이 아니다.

| CSV                                      | 제외 이유                                                                                                                                                                                                                                 |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `olist_geolocation_dataset.csv`          | 유일한 Join Key `geolocation_zip_code_prefix`에 대응하는 `customer_zip_code_prefix`/`seller_zip_code_prefix`가 Source Column Allowlist 밖이다. 또한 1,000,163행이 19,015개 Prefix에 중복 분포해 `zip_code_prefix`가 PK로 Unique하지 않고, Cursor Timestamp Column도 없다. 포함하려면 Bronze Schema 변경과 7개 Table 재수집, PK·Cursor 계약 예외가 함께 필요하다. |
| `olist_order_reviews_dataset.csv`        | Star Schema의 주문·결제·배송 Measure 계약에 참여하지 않는다. 별도 Review Fact를 요구하며 V1 Mart Grain 밖이다.                                                                                                                                    |
| `product_category_name_translation.csv`  | `products.product_category_name`을 원본 포르투갈어 그대로 보존하는 4.2 보존 원칙과 별개인 표시용 번역 Mapping이다. Dimension Attribute 번역은 V1 Mart 범위 밖이다.                                                                              |
...
| customer_subscriptions | 구독 계약·자동갱신 상태·청구 일정·상태 시각, `created_at`, `updated_at` | 구독 계약 운영과 증분 수집 |
| customer_membership_tiers | `membership_tier`, `created_at`, `updated_at` | 사람 단위 거래 실적 등급과 증분 수집 |
| subscription_payments | 결제 식별자·청구 회차·재시도·결제/청구 시각·금액·결제수단, `created_at`, `updated_at` | 구독 자동결제 이력과 증분 수집 |
...
| customer_subscriptions | Seed 없음. 구독 계약 시작 시 Generator가 생성 |
...
Seed는 Olist 원본에 구독 이력이 없으므로 `customer_subscriptions` 행을 만들지 않는다.
구독 계약이 없는 고객이 비구독 상태다. `membership_tier`는
...
```text
2026-01-05  구독 계약 ACTIVE 시작    updated_at = 01-05
2026-02-01  등급 BRONZE→SILVER      updated_at = 02-01   -- 구독은 안 바뀜
```

`CANCEL_REQUESTED` 상태에서 등급만 바뀌면 `updated_at`이 앞으로 밀리는데 `current_period_ends_at`은
...
Grain은 구독 계약 1건이다. 계약이 없는 고객은 이 테이블에 행이 없으며, 해지 뒤 재가입하면
새 `subscription_id` 계약을 만든다.

| Column | Type | Constraint |
| --- | --- | --- |
| `subscription_id` | UUID | PK, 구독 계약 식별자 |
| `customer_unique_id` | VARCHAR(64) | NOT NULL, 사람 Business Key |
| `subscription_status` | VARCHAR(32) | `ACTIVE/PAYMENT_FAILED/CANCEL_REQUESTED/CHURNED` |
| `auto_renew_enabled` | BOOLEAN | NOT NULL, 자동갱신 허용 여부 |
| `subscription_started_at` | TIMESTAMPTZ | NOT NULL, 계약 시작 시각 |
| `current_period_started_at`, `current_period_ends_at` | TIMESTAMPTZ | 현재 혜택 기간 |
| `billing_due_at` | TIMESTAMPTZ | 현재 청구 회차의 원래 청구 기한 |
| `next_payment_attempt_at` | TIMESTAMPTZ | 정기 청구 또는 실패 재시도의 실제 다음 실행 예정 시각 |
| `payment_failed_at`, `cancel_requested_at`, `ended_at` | TIMESTAMPTZ | 최근 실패·해지 신청·실제 종료 시각 |
| `status_changed_at` | TIMESTAMPTZ | NOT NULL, 현재 상태 전이 시각 |
| `created_at`, `updated_at` | TIMESTAMPTZ | NOT NULL, 원천 생성·변경 및 증분 Cursor 시각 |
...
Grain은 구독 계약의 청구 회차별 결제 시도 1건이다. 실패 후 재시도는 같은 청구 회차에서
`attempt_sequence`만 증가한다.

| Column | Type | Constraint |
| --- | --- | --- |
| `payment_id` | UUID | PK, 결제 시도 식별자 |
| `subscription_id` | UUID | NOT NULL, FK → `customer_subscriptions` |
| `billing_cycle_sequence`, `attempt_sequence` | INTEGER | NOT NULL, 모두 1 이상, 계약·회차·시도 조합 Unique |
| `payment_status` | VARCHAR(16) | `completed/failed` |
| `payment_at` | TIMESTAMPTZ | NOT NULL, 실제 결제 시도·결과 확정 시각 |
| `payment_value`, `currency_code` | NUMERIC(14,2), CHAR(3) | NOT NULL, 청구 금액과 ISO 통화 코드 |
| `billing_period_start_at`, `billing_period_end_at` | TIMESTAMPTZ | NOT NULL, 적용 혜택 기간, 종료가 시작보다 뒤 |
| `payment_method_type`, `payment_provider`, `provider_payment_id` | VARCHAR | NULL, 결제수단·대행사·대행사 거래 ID |
| `failure_code` | VARCHAR(64) | 실패 원인 코드. 성공이면 `NULL` |
| `created_at`, `updated_at` | TIMESTAMPTZ | NOT NULL, 원천 생성·변경 및 증분 Cursor 시각 |
...
`customer_membership_tiers` Bronze 누적 행에서 별도로 만든다. 두 축을 Warehouse에서 어떻게
병합해 고객 이력으로 만드는지는 `docs/reference/mart-grain.md`가 정본이다.
...
Intermediate는 Staging과 Mart 사이에서 Grain을 맞추는 계층이다. Join과 사전 집계를 여기서
끝내고, Mart는 준비된 행을 투영한다.

PRD 수준에서 고정하는 규칙은 다음과 같다.

- Grain이 다른 입력을 Fact에서 직접 다대다 Join하지 않는다. Intermediate에서 목표 Grain으로
  먼저 접는다.
- 두 Source의 증분 Watermark가 독립적이라 한쪽만 새 데이터가 도착하는 경우가 정상적으로
  발생한다. Intermediate가 이 부분 결측을 흡수하고 Fact까지 번지지 않게 한다.
- 집계·파생·이력 구간 생성은 Intermediate에서 끝낸다. Fact와 Dimension은 계산 책임을 갖지
  않는다.

Intermediate Model 목록과 각 Model의 입력·출력 Grain은 `docs/reference/mart-grain.md`와
`docs/reference/data-transformation-flow.md`가 정본이다.

### 14.3 Mart Grain과 Measure 계약

Grain은 "한 행이 무엇 하나를 나타내는가"의 선언이다. Model마다 Grain 문장을 정의로 삼고,
Unique Key는 그 문장을 데이터로 강제하는 수단이다. Measure는 자신이 속한 Grain에서만
유효하다.

Dimension, Fact, Report Model의 Grain·Unique Key 목록과 Model별 Grain 문장, Measure
계산식, `NULL` 처리 규칙, 새 Model 추가 시 Grain 심사 기준은 `docs/reference/mart-grain.md`가
단일 정본이다. 이 PRD는 그 계약을 중복 정의하지 않는다.

PRD 수준에서 고정하는 불변 규칙은 다음과 같다.
...
- Delivery Measure 4개는 Non-additive다. SUM 금지, 평균·분위수·비율로만 집계한다.
- Grain이 같은 두 Fact는 한 Table로 유지한다. 배송 Timestamp는 주문 1건당 각 1개이므로
  Delivery Measure는 `fact_orders`에 함께 두고 별도 `fact_delivery`를 만들지 않는다.
...
사람 단위 속성은 `UPDATE`로 덮어쓰는 Source에서 과거 값이 남지 않는다. Bronze는 Batch마다
새 Object를 쌓아 관측 이력을 보존하므로, Warehouse는 이 누적 관측에서 시점별 유효 값을
복원해야 한다.
...
PRD 수준에서 고정하는 요구사항은 다음과 같다.

- 사람 단위 속성의 변경 이력을 시점별로 조회할 수 있어야 한다. 현재 값으로 과거 사실을
  다시 해석하지 않는다.
- 상태 변화가 아닌 운영 속성은 이력 Version을 늘리지 않는다. `billing_due_at`과
  `next_payment_attempt_at`은 구독 상태가
  바뀌지 않아도 매달 달라지므로 Version 생성 조건에서 제외한다. 현재 값은 최신 Version에서
  조회한다.
- Olist Seed는 과거 속성 이력이 없으므로 최초 Version을 Baseline Snapshot으로 간주한다.
- 같은 사람의 같은 관측 시각에서 서로 다른 속성 값이 관측되면 Contract 위반이다.
- Fact는 사건 발생 시점에 유효했던 Version을 참조한다. 정상 E2E에서 유효 Version을 찾지 못한
  Unknown Fact Count는 0이어야 한다.

이 요구사항을 만족하는 구체적 구현 방식(SCD Type 2 Version 구간 생성 규칙, Surrogate Key
계산식, `dim_customer` Schema, 추적 속성 목록, Temporal Join 조건)은
`docs/reference/mart-grain.md`가 정본이다.
...
정상 E2E Unknown 참조 = 0
표준화 상태값 매핑 누락 = 0
Grain이 다른 입력 결합으로 인한 Fact Fan-out = 0
```
