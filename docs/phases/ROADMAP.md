...
Phase 7~10은 각 문서 안에서 Task를 두 단계로 나눈다.

| 단계     | 의미                                                | 착수 시점        |
| -------- | --------------------------------------------------- | ---------------- |
| `N-1` 골격 | Mart 구성과 무관한 Task. Ingestion 계층, Harness, 전환 메커니즘, 연결 검증 | Phase 6 완료 전 |
| `N-2` 적용 | Model 이름과 Grain이 확정돼야 하는 Task             | Phase 6 완료 후 |

Phase 6이 Grain 계약 확정을 담당하므로 이후 Phase의 상당수 Task가 여기에 묶인다. 골격 Task를 먼저 진행하면 Grain 설계와 병행할 수 있다. 골격 단계에서 Mart 목록이 필요한 자리는 설정으로 분리하고 Phase 6 확정 후 주입한다.

Phase 순서 자체는 바꾸지 않는다. "품질 → 장애 → 성능 → 소비"라는 검증 흐름은 유지하고, 각 Phase 내부에서만 착수 시점을 나눈다.
