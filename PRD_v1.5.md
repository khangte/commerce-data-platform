...
v1.5는 v1.4의 Source 원본 보존, 증분 수집, Bronze 불변성, SCD2, Late Arrival, 재처리 계약을 유지하면서 주문 상태의 분석 경계를 명확히 한다.

- Staging의 `order_status`는 원천 8개 상태를 대문자로만 표준화해 세부 운영 상태를 보존한다.
- 다중 상태를 묶는 분석은 물리 컬럼이 아니라, 반복 사용될 때만 dbt Macro 또는 Mapping Seed로 집계 시점에 재사용한다.

v1.4에서 확정한 다음 계약은 유지한다.
